...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    try {
        ...
        let randomkamu = `${Math.floor(Math.random() * 91)}`.trim() //hehe Biar Susah Menang :v
        let Aku = (randomaku * 1)
        ...
        } else conn.reply(m.chat, `Uang kamu tidak mencukupi untuk Casino silahkan cari uang terlebih dahulu!`.trim(), m)
    } catch (e) {
        console.log(e)
        m.reply('Error!!')
        if (DevMode) {
