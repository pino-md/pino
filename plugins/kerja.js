...
`.trim()
            ...
            setTimeout(() => {
                  conn.sendMessage(m.chat, { text: str3 }, { quoted: m })
                  }, 15000)
            setTimeout(() => {
            ...
            } else conn.reply(m.chat, `Anda sudah bekerja hari ini dan kelelahan saat bekerja, silahkan coba *${timers}* lagi`, m)
        } else conn.reply(m.chat, 'Minimal 80 health untuk bekerja, beli nyawa dulu dengan ketik *' + usedPrefix + 'shop buy potion <jumlah>*\ndan ketik *' + usedPrefix + 'use potion <jumlah>*', m)
    } catch (e) {
        console.log(e)
...
handler.help = ['kerja']
handler.tags = ['rpg']
handler.command = /^(kerja|work|bekerja|ker(ja)|wo(rk))$/i
handler.limit = true
handler.group = true
handler.fail = null
